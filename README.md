...
This is a fork of [balmasi/migrate-mongoose](https://github.com/balmasi/migrate-mongoose) with two changes:

* No throw error when not exists migrations to run and try 'up' command
* Upgrade mongoose version from 4 to 5.
...
```
 npm install custom-migrate-mongoose
```
and then run
```
...
### Usage
```
Usage: ./node_modules/.bin/migrate -d <mongo-uri> [[create|up|down<migration-name>]|list|prune] [optional options]
...
./node_modules/.bin/migrate list --config somePath/myCustomConfigFile[.json]
```
